        ...
        <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
          {categories.map((category, index) => (
            <Link key={index} href={category.link} className="perspective-1000">
              ...
              >
                {/* SVG Illustration */}
                <div className="absolute inset-0 z-0 flex items-center justify-center p-6 overflow-hidden">
                  {/* Gradient background */}
                  <div className={`absolute inset-0 bg-gradient-to-br ${category.fallbackColor} opacity-5 dark:opacity-10`}></div>
                  ...
                  <div className="absolute inset-0 flex items-center justify-center p-6">
                    <div className="w-full h-full flex items-center justify-center">
                      <img
                        src={category.image}
                        alt={category.title}
                        className="w-full h-full object-contain opacity-20 dark:opacity-30 transform scale-110 group-hover:scale-125 transition-all duration-500"
                      />
                    </div>
                  </div>
                </div>
                ...
                <div className="absolute inset-0 bg-white/80 dark:bg-zinc-800/80 z-5"></div>

                {/* Content container */}
                <div className="absolute inset-0 z-10 p-6 flex flex-col justify-between">
                  {/* Glowing orb with icon */}
                  <div
                    className={`relative h-16 w-16 rounded-full flex items-center justify-center transition-all duration-500 group-hover:scale-110 text-white ${category.glowColor === 'blue' ? 'bg-blue-500 dark:bg-blue-500 ring-blue-200 dark:ring-blue-500/30' :
                        category.glowColor === 'purple' ? 'bg-purple-500 dark:bg-purple-500 ring-purple-200 dark:ring-purple-500/30' :
                          category.glowColor === 'emerald' ? 'bg-emerald-500 dark:bg-emerald-500 ring-emerald-200 dark:ring-emerald-500/30' :
                            'bg-amber-500 dark:bg-amber-500 ring-amber-200 dark:ring-amber-500/30'
                      } ring-2`}
                  >
                    {category.icon}
                  </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center justify-between pt-2">
                      ...
                      <span className={`rounded-full p-2 transition-all duration-300 border group-hover:scale-110 ${category.glowColor === 'blue' ? 'bg-blue-100 dark:bg-blue-500/30 text-blue-600 dark:text-blue-100 border-blue-200 dark:border-blue-500/30' :
                          category.glowColor === 'purple' ? 'bg-purple-100 dark:bg-purple-500/30 text-purple-600 dark:text-purple-100 border-purple-200 dark:border-purple-500/30' :
                            category.glowColor === 'emerald' ? 'bg-emerald-100 dark:bg-emerald-500/30 text-emerald-600 dark:text-emerald-100 border-emerald-200 dark:border-emerald-500/30' :
                              'bg-amber-100 dark:bg-amber-500/30 text-amber-600 dark:text-amber-100 border-amber-200 dark:border-amber-500/30'
                        }`}>
