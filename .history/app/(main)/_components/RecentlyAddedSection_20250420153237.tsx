...
const CourseCard = ({ course }: {
  ...
  const estimatedHours = Math.max(3, Math.min(20, (course?.chapters.length || 1) * 1.5));
    ...
    <Card className="flex flex-col w-full overflow-hidden transition-all duration-300 hover:shadow-lg dark:bg-zinc-900 dark:border-zinc-800 group h-full">
      <div className="relative w-full aspect-video overflow-hidden">
        <Link href={`/courses/${course?.id}`}>
          ...
          {course?.price && course.price > 0 && course.price < 2000 && (
          ...
          {course?.price !== undefined && (
            <div className="text-sm font-semibold">
              {course.price === 0 ? (
                ...
                <span className="text-primary dark:text-primary-foreground">{formatPrice(course.price)}</span>
              )}
            </div>
          )}
        </div>
